...
public class HostBridge : IAsyncDisposable
{
    ...
    private bool? _isEmbeddedcached;
    ...
    public async Task<bool> IsEmbeddedAsync()
    {
        if (_isEmbeddedcached.HasValue)
            return _isEmbeddedcached.Value;

        _isEmbeddedcached = await _js.InvokeAsync<bool>(
            "blazorHostBridge.isInIFrame");
